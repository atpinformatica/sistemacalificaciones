...
async function guardarEnGoogleSheets() {
    ...
    alumnos.forEach((a, i) => {
        ...
        datos.push({
            _url: URL_WEB_APP, // Control del Service Worker
            n_orden: i + 1, 
            nombre: a.nombre, 
            curso, turno, materia, 
    ...
    // CONTROL OFFLINE: Si no hay señal de red
    if (!navigator.onLine) {
        try {
            // Enviamos el fetch simulado. El Service Worker lo va a atajar y guardar en IndexedDB
            await fetch(URL_WEB_APP, { 
                method: 'POST', 
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(datos)
            });
            alert("⚠️ Guardado en Modo Offline: No tienes internet, pero las notas se guardaron en la memoria del celular. Se enviarán solas a Google Sheets apenas recuperes conexión.");
        } catch(e) {
            console.error("Error en ruteo offline interno:", e);
        }
    ...
    // CONTROL ONLINE: Envío clásico directo a Google
    try {
