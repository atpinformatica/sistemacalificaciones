...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if ('serviceWorker' in navigator) {
        ...
        // Escuchar mensajes del SW (ej: sincronización completada)
        navigator.serviceWorker.addEventListener('message', event => {
            if (event.data === 'SYNC_COMPLETADA') {
                mostrarBannerSincronizado();
            }
        });
    }

    // Cuando vuelve la conexión, disparar sincronización manual
    window.addEventListener('online', () => {
        ...
        // También intentar Background Sync si está disponible
...
function mostrarEstadoConexion(online) {
    ...
    if (online) {
        ...
        banner.innerHTML = '✅ Conectado — los datos pendientes se están sincronizando...';
        setTimeout(() => { banner.style.display = 'none'; }, 4000);
    } else {
        banner.style.display = 'block';
        banner.style.background = '#e74c3c';
        banner.style.color = 'white';
        banner.innerHTML = '⚠️ Sin conexión — podés seguir cargando notas, se guardarán y enviarán cuando vuelva internet';
    }
}
...
function mostrarBannerSincronizado() {
    ...
    if (banner) {
        banner.style.background = '#27ae60';
        banner.innerHTML = '✅ ¡Datos sincronizados con Google Sheets!';
        setTimeout(() => { banner.style.display = 'none'; }, 4000);
    }
}

// 5. GUARDAR DATOS — con soporte offline
async function guardarEnGoogleSheets() {
    ...
    alumnos.forEach((a, i) => {
        // CORRECCIÓN CLAVE: Buscamos en la memoria local por NOMBRE, no por DNI
        const m = memoriaGlobal[llave] ? (memoriaGlobal[llave][a.nombre] || {}) : {};
        
        datos.push({
            _url: URL_WEB_APP, // Control indispensable para el Service Worker
            n_orden: i + 1, 
            nombre: a.nombre, 
            curso, turno, materia, 
    ...
    // CONTROL OFFLINE: Si el navegador detecta que no hay internet
    if (!navigator.onLine) {
        try {
            // Enviamos la petición. El sw.js la interceptará y la guardará en la cola IndexedDB
            await fetch(URL_WEB_APP, { 
                method: 'POST', 
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(datos)
            });
            alert("⚠️ Guardado en Modo Offline: No tienes internet. Las notas se guardaron localmente en el celular y se subirán solas a Google Sheets apenas recuperes conexión.");
        } catch(e) {
            console.error("Error en enrutamiento offline:", e);
        }
    ...
    // CONTROL ONLINE: Envío clásico directo a Google Sheets
    try {
        ...
        alert("¡Datos guardados con éxito directamente en Google Sheets!");
    } catch (e) { 
        console.error(e);
        alert("Error de red al intentar guardar. Verifique su conexión."); 
    }
...
// Detectar el regreso de internet y forzar el vaciado de la cola offline
window.addEventListener('online', () => {
    console.log("Conexión restablecida. Avisando al Service Worker para sincronizar...");
    if (navigator.serviceWorker && navigator.serviceWorker.controller) {
        // Enviamos una señal al sw.js para que despache lo que tenga acumulado
        navigator.serviceWorker.controller.postMessage('SYNC_NOW');
    }
});

// Escuchar si el Service Worker terminó de subir los datos para avisarle al usuario
navigator.serviceWorker.addEventListener('message', event => {
    if (event.data === 'SYNC_COMPLETADA') {
        alert("¡Sincronización Automática Exitosa! Los datos que cargaste sin internet ya se subieron a Google Sheets.");
        validarYFiltrar(); // Refresca la vista en pantalla
    }
});