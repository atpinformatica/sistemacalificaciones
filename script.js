...
async function guardarEnGoogleSheets() {
    ...
    if (!navigator.onLine) {
        // SIN INTERNET: guardar en IndexedDB via SW
        alert("⚠️ Sin conexión. Los datos se guardaron localmente y se enviarán automáticamente cuando vuelva internet.");
        btn.innerHTML = '<i class="fas fa-cloud-upload-alt"></i> Guardar Datos';
        btn.disabled = false;
        // El fetch va a fallar y el SW lo intercepta y guarda en la cola
    }

    try {
        // Con internet: envío normal (el SW deja pasar el POST)
        // Sin internet: el SW intercepta y guarda en IndexedDB
        ...
        if (navigator.onLine) alert("¡Datos guardados con éxito!");
    } catch (e) { 
        console.error(e);
        if (navigator.onLine) alert("Error de red al intentar guardar"); 
    }
